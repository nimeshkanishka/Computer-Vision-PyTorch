...
for img_file in os.listdir(IMAGE_FOLDER):
    try:
        ...
        # If the network requires grayscale images, load the image in grayscale format
        if not RGB:
            img_array = cv2.imread(os.path.join(IMAGE_FOLDER, img_file), cv2.IMREAD_GRAYSCALE) # (height, width)

        # img_array will have 3 dims if the image is loaded in RGB format or 2 dims if the image is loaded in grayscale format
...
for batch_start in range(0, X.shape[0], BATCH_SIZE):
    ...
    for i in range(batch_end - batch_start):
        ...
        print(f"Image: {batch_start + i} - Prediction: {pred} - Confidence: {confidence:.2f}%")
