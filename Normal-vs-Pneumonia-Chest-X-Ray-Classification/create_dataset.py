    ...
    while num_images < num_images_per_class:
        ...
        for img_file in img_files[:num_images_to_add if num_images_to_add < len(img_files) else len(img_files)]:            
            try:
                ...
                # Load the image
                if RGB:
                    img_array = cv2.imread(os.path.join(img_folder, img_file), cv2.IMREAD_COLOR) # (img_size, img_size, num_channels)
                    img_array = cv2.cvtColor(img_array, cv2.COLOR_BGR2RGB) # (img_size, img_size, num_channels)
                else:
                    img_array = cv2.imread(os.path.join(img_folder, img_file), cv2.IMREAD_GRAYSCALE) # (img_size, img_size)
                    img_array = np.expand_dims(img_array, axis=2) # (img_size, img_size, num_channels)
