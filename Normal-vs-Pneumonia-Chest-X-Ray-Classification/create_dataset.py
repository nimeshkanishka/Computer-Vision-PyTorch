...
# Calculate mean and std of training data for normalization (if using a custom network)
if RGB:
    # Channelwise mean and std for RGB images
    mean = (X.astype(np.float32) / 255.0).mean(axis=(0, 1, 2)) # (3,)
    std = (X.astype(np.float32) / 255.0).std(axis=(0, 1, 2)) # (3,)
else:
    # Single mean for grayscale images
    mean = (X.astype(np.float32) / 255.0).mean() # Scalar
    std = (X.astype(np.float32) / 255.0).std() # Scalar

np.savez(f"Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Training-Dataset-{IMAGE_SIZE}x{IMAGE_SIZE}-{'RGB' if RGB else 'Gray'}.npz",
...
np.savez(f"Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Validation-Dataset-{IMAGE_SIZE}x{IMAGE_SIZE}-{'RGB' if RGB else 'Gray'}.npz",
         X=X, y=y)
...
np.savez(f"Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Test-Dataset-{IMAGE_SIZE}x{IMAGE_SIZE}-{'RGB' if RGB else 'Gray'}.npz",
         X=X, y=y)
