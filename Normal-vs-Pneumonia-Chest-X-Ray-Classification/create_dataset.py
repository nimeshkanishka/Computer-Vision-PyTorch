    ...
    while num_images < num_images_per_class:
        ...
        for img_file in img_files[:num_images_to_add if num_images_to_add < len(img_files) else len(img_files)]:            
            try:
                ...
                img_array = cv2.imread(os.path.join(img_folder, img_file), cv2.IMREAD_GRAYSCALE)
...
np.savez(f"Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Training-Dataset.npz", X=X_train, y=y_train)

print(f"----- Training dataset saved to 'Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Training-Dataset.npz' -----")
...
np.savez(f"Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Validation-Dataset.npz", X=X_val, y=y_val)

print(f"----- Validation dataset saved to 'Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Validation-Dataset.npz' -----")
...
np.savez(f"Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Test-Dataset.npz", X=X_test, y=y_test)

print(f"----- Test dataset saved to 'Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Test-Dataset.npz' -----")