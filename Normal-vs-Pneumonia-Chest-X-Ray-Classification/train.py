...
match NETWORK:
    case "Custom":
        ...
        # Increase the learning rate from 4e-5 to 1e-3 within the first 30% of steps and then anneals to 1e-7
    ...
    case "EfficientNet-B0":
        ...
        # AdamW optimizer applies weight decay (L2 regularization) and adapts learning rates per parameter
        optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
        # Increase the learning rate from 2e-5 to 5e-4 within the first 20% of steps and then anneals to 5e-8
