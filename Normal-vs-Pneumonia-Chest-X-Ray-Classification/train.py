...
dataset_train = np.load("Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Training-Dataset.npz")
X_train = dataset_train["X"] # (num_images, img_size, img_size, num_channels)
y_train = dataset_train["y"] # (num_images,)

# Load validation dataset
dataset_valid = np.load("Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Validation-Dataset.npz")
X_val = dataset_valid["X"] # (num_images, img_size, img_size, num_channels)
y_val = dataset_valid["y"] # (num_images,)

# Load test dataset
dataset_test = np.load("Normal-vs-Pneumonia-Chest-X-Ray-Classification/Data/Test-Dataset.npz")
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch > 9) and ((val_acc > best_val_acc) or ((val_acc == best_val_acc) and (val_loss <= best_val_loss))):
        ...
        torch.save(model.state_dict(),
                   "Normal-vs-Pneumonia-Chest-X-Ray-Classification/Training/Model_Checkpoint.pth")
        
        print(f"----- Model checkpoint saved to 'Normal-vs-Pneumonia-Chest-X-Ray-Classification/Training/Model_Checkpoint.pth' -----")
...
plt.savefig("Normal-vs-Pneumonia-Chest-X-Ray-Classification/Training/Training_Graph.png")
plt.show()
...
model.load_state_dict(torch.load("Normal-vs-Pneumonia-Chest-X-Ray-Classification/Training/Model_Checkpoint.pth"))
model.eval()
