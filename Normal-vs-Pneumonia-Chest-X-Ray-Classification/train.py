...
match NETWORK:
    case "Custom":
        ...
        # Apply weight decay (L2 regularization) and adapt learning rates per parameter
        optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
        # Increase the learning rate from 2e-5 to 5e-4 within the first 30% of steps and then anneals to 5e-8
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=5e-4, epochs=NUM_EPOCHS,
    ...
    case "EfficientNet-B0":
        ...
        # Apply weight decay (L2 regularization) and adapt learning rates per parameter
        optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
        # Increase the learning rate from 4e-6 to 1e-4 within the first 20% of steps and then anneals to 1e-8
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-4, epochs=NUM_EPOCHS,
                                                  steps_per_epoch=X_train.shape[0] // BATCH_SIZE, pct_start=0.2)
...
val_dataset = TensorDataset((torch.from_numpy(X_val).permute(0, 3, 1, 2).float() / 255.0 - mean) / std,
                            torch.from_numpy(y_val).float())
test_dataset = TensorDataset((torch.from_numpy(X_test).permute(0, 3, 1, 2).float() / 255.0 - mean) / std,
...
for epoch in range(NUM_EPOCHS):
    ...
    # Save model checkpoint if we have completed more than 5 epochs AND validation accuracy has increased or
    # validation loss has decreased with validation accuracy remaining the same
    if (epoch > 4) and ((val_acc > best_val_acc) or ((val_acc == best_val_acc) and (val_loss <= best_val_loss))):
